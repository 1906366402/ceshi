...
class TagController extends Controller {
    ...
    public function index(){
        ...
        //S('tagPagePC',null);  //删除缓存
        if(S('tagPagePC')){
            echo S('tagPagePC');
        }else{
            $tagArray2 = $tagModel->query('select id,tag from ls_seo_tagindex order by id desc limit 3000');

            /*foreach ($tagArray2 as $key=>$value) {
                if(strlen($value[$key]['tag'])<25){
                    $k=0;
                    $tagArray6[$k]['id']= $value[$key]['id'];
                    $tagArray6[$k]['tag']=$value[$key]['tag'];
                    $k++;
                }    
            }
            dump($tagArray6);die;*/
            ...
            //不显示8个字以上的
            for ($i=0; $i < count($tagArray2); $i++) { 
                if(strlen($tagArray2[$i]['tag'])>24){
                    /*$tagArray6[$i]['id'] = $tagArray2[$i]['id'];
                    $tagArray6[$i]['tag'] = $tagArray2[$i]['tag'];*/
                    unset($tagArray2[$i]);
                }
            }
    ...
    public function taglist(){
        ...
        $pageObj->rollPage=5;
        $pageObj->url = 'http://m.liansuo.com/tag/'.$newsid.'/p'.$page_tpl.'.html';
