...
class MapController extends Controller
{
	...
	public function projectSetMap()
    {
		...
		$fo=fopen(APP_PATH . '../../Web/sitemap/list-liansuo-com/pro301.txt','r+');
		foreach($templist as $one){
			fwrite($fo,$one['ypurl'].' '.'http://www.liansuo.com/p'.$one['memberid']."/\r");			
	...
	public function mprojectSetMap()
    {
        header('Content-type:text/html;charset=utf-8');
        // 获取问题列表
        // $list = self::$questionObj->getQuestionSetMap($where, $maxid, $psize);
        ...
        $xml.='</urlset>'; 
		$filename = APP_PATH . '../../Web/sitemap/list-liansuo-com/mproject.xml';
        if (! file_put_contents($filename, $xml))
            exit("写入数据错误");
        else
            echo "写入数据成功<br />";
