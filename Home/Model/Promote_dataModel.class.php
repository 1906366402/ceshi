...
class Promote_dataModel extends Model
{
    ...
    public function getAdList($pararr){
		...
		if(isset($tag['cid'])){//原来的系统			
			...
			$tempar=$this->bannerID->query('select b.*,b.member_id as memberid from ls_ad_weburl as a left join ls_ad_banner as b on a.bannerID=b.id where a.channelID='.$tag['cid'].' order by a.id asc limit '.$row);
			if(count($tempar)>0){
		...
		if($tag['source']){
			...
			$temparrm=$this->resources_memory->field('memberid')->where($swhere)->limit($row)->select();
			if(count($temparrm)>0){
