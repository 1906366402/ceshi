...
class News_arcModel extends Model
{
    ...
    public function getArcList($pararr)
    {
		...
		if (isset($tag['notcid'])) {
              if (strpos($tag['notcid'], ',') === false) {
                $tempstrc = ' cid <>' . intval($tag['notcid']);
            } else {
                $tempstrc = ' cid not in(' . $tag['notcid'] . ')';
            }
			 $where .= ' and ' . $tempstrc;
		 }
        ...
        if (isset($tag['pid'])) {
		 ...
		 // 实例分类
		if(!$this->class_industry){
			$this->class_industry = M("class_industry");// 实例化class_industry对象
		}     
		///配置了行业属性
		if(isset($tag['industry'])){ 
			$temppid = explode(',', $tag['industry']);
			 $temparri = json_decode($this->arcResit->redis->hget('category_lib','category'), true);		 
			 $temparria=array();			 
			 foreach($temparri as $keyi=>$abc){
				 if(in_array($abc['b_industry'],$temppid)||in_array($abc['s_industry'],$temppid)){
					 $temparria[]=$keyi;
				 }
	/*			if($tag['industry']==$abc['b_industry']||$tag['industry']==$abc['s_industry']){
					$temparria[]=$keyi;
				}*/
			 }		 
			 if(count($temparria)>1){
				  $where .= ' and cid in(' . join(',', $temparria) . ')';			
			 }else{
				 $tempp_id='';
				 foreach($temppid as $v){
					 $pid=$this->class_industry->where("id=".$v)->find();
					 $tempp_id[] = $pid["parentid"];
				 }
				 foreach($temparri as $keyi=>$abc){
				 if(in_array($abc['b_industry'],$tempp_id)||in_array($abc['s_industry'],$tempp_id)){
					 $temparria[]=$keyi;
				 }
				 if(count($temparria)>1){
					$where .= ' and cid in(' . join(',', $temparria) . ')';			
        ...
        if ($tag['source']) {
            ...
            if ($tempnewsid) {
				...
				//echo $this->source_content->getLastSQL();die;
				$newssour=array();
