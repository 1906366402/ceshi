...
class TagController extends Controller {
    ...
    public function index(){
        ...
        $tagModel = new Model();
            ...
            echo $this->buildHtml('index.html', DOCUMENT_ROOT_DIR.'/tag/','index');
    }
    ...
    public function taglist(){
        ...
        if(empty($taglistArrayIndex)){
            header('HTTP/1.1 404 Not Found');
            include('/data/www/www.liansuo.com/header_footer/error.html');die; 
        }
        //取出id然后在下面not in 掉，不然会重复
        foreach($taglistArrayIndex as $one){
            $taglistArrayKey2[] = $one['aid'];
        }
        $taglistArrayKey = implode(',',$taglistArrayKey2);
        $tagindex = $taglistModel->query("select catalog_id,tag from ls_seo_taglist where tid=$newsid order by aid desc ");
        $tagindex['catalog_id'] = $tagindex[0]['catalog_id'];
        $tagindex['tag'] = $tagindex[0]['tag'];
        $where2['catalog_id'] = $tagindex['catalog_id'];
        $where2['flag'] = array('in',array(1,2));
        $where2['iscreatehtml'] = array('in',array(1,2));
        $where2['newsid'] = array('not in',$taglistArrayKey);
        $arclistArray = new Model('news_arc');
        $taglistArrayPush = $arclistArray->field('newsid aid')->where($where2)->order('ctime desc')->limit(150)->select();
        ...
        //$this->buildHtml($newsid.'.html', DOCUMENT_ROOT_DIR.'/tag/','taglist');
        $this->display();
    }
} 
